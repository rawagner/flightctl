	...
	err = repotester.TestAccess(&model.Repository{
		Spec: &model.JSONField[api.RepositorySpec]{
			Data: api.RepositorySpec{
				Repo: util.StrToPtr("https://localhost:4443"),
				HttpConfig: &api.RepositoryHttpConfig{
					TlsClientCertKey:  &clientKeyB64,
					TlsClientCertData: &clientCrtB64,
					RootCA:            &caB64,
				},
			},
		},
...
func TestSSHRepo(t *testing.T) {
	...
	err = repotester.TestAccess(&model.Repository{
		Spec: &model.JSONField[api.RepositorySpec]{
			Data: api.RepositorySpec{
				Repo: util.StrToPtr("ssh://root@127.0.0.1:2222"),
				SshConfig: &api.RepositoryGitConfig{
